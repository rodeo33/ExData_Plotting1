...
with(edata,plot(dt,Voltage
                ,ylab = "Voltage"
                ,xlab="datetime",typ="n"))
with(edata,lines(dt,Voltage))
...
##add legend
legend("topright",lwd=0.2,lty=1,col=c("black","red","blue"),
       legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),cex=0.55)

##plot global reactive power graph

with(edata,plot(dt,Global_reactive_power
                ,ylab = "Global Reactive Power"
                ,xlab="datetime",typ="n"))
with(edata,lines(dt,Global_reactive_power))
